...
export function DeveloperStagingSuite() {
  ...
  return (
    <section className="rounded-3xl border border-white/[0.08] bg-[#111815] p-5 sm:p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2"><Code2 className="h-4 w-4 text-[#b3ff3e]" /><p className="text-sm font-medium text-white">Developer staging suite</p></div>
        ...
        <div className="flex items-center gap-2"><Badge className={cn("border-0", enabled ? "bg-[#b3ff3e]/10 text-[#d8ff9d]" : "bg-white/[0.06] text-white/45")}>{enabled ? "Dev mode on" : "Dev mode off"}</Badge><Switch checked={enabled} onCheckedChange={setEnabled} aria-label="Enable developer staging mode" /></div>
      </div>
      ...
      {enabled ? <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div className="rounded-2xl border border-white/[0.08] bg-black/15 p-4">
          ...
          <div className="mt-2 flex gap-2"><Input id="staging-url" value={url} onChange={event => setUrl(event.target.value)} placeholder="http://localhost:5173" className="h-10 border-white/[0.10] bg-white/[0.04] text-sm text-white placeholder:text-white/25" /><Button asChild variant="outline" className="h-10 shrink-0 border-white/[0.10] bg-transparent px-3 text-white/70 hover:bg-white/[0.06] hover:text-white"><a href={previewUrl || undefined} target="_blank" rel="noreferrer" aria-disabled={!previewUrl}><ExternalLink className="h-3.5 w-3.5" /></a></Button></div>
        ...
        <aside className="space-y-3">
          <div className="rounded-2xl border border-[#b3ff3e]/15 bg-[#b3ff3e]/[0.04] p-4"><WalletCards className="h-4 w-4 text-[#b3ff3e]" /><p className="mt-3 text-sm font-medium text-white">Simulation contract</p><p className="mt-1 text-xs leading-5 text-white/42">The frame receives a <code className="text-[#d8ff9d]">serapay:staging-context</code> message after load with a labelled test address, chain ID, and test USDC balance.</p><Button onClick={sendSimulatorContext} variant="outline" className="mt-4 h-9 w-full border-white/[0.10] bg-transparent text-xs text-white/70 hover:bg-white/[0.06] hover:text-white">Resend test context</Button></div>
          <div className="rounded-2xl border border-white/[0.08] bg-white/[0.025] p-4"><ShieldCheck className="h-4 w-4 text-[#b3ff3e]" /><p className="mt-3 text-sm font-medium text-white">Boundary by design</p><p className="mt-1 text-xs leading-5 text-white/42">Preview frames run in a restricted sandbox. Production release still requires manifest validation and owner approval.</p></div>
        </aside>
