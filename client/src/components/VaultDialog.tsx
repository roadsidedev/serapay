...
export function VaultDialog({ address, open, onOpenChange, tokens, onSubmitted }: VaultDialogProps) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-md border-white/10 bg-[#111916] text-white"><DialogHeader><DialogTitle>Sera Vault</DialogTitle><DialogDescription className="text-white/45">{mode === "deposit" ? "Deposit using an ERC-2612 permit when supported, otherwise approve then deposit." : "Withdraw through Sera’s dual-signature instant-withdrawal flow."}</DialogDescription></DialogHeader><div className="mt-2 grid grid-cols-2 rounded-xl bg-white/[0.04] p-1 text-xs"><button onClick={() => { setMode("deposit"); setReviewing(false); setStage("review"); }} className={`rounded-lg px-3 py-2 ${mode === "deposit" ? "bg-[#b3ff3e] font-semibold text-[#0b1a0e]" : "text-white/50"}`}>Deposit</button><button onClick={() => { setMode("withdraw"); setReviewing(false); setStage("review"); }} className={`rounded-lg px-3 py-2 ${mode === "withdraw" ? "bg-[#b3ff3e] font-semibold text-[#0b1a0e]" : "text-white/50"}`}>Withdraw</button></div><div className="space-y-4 pt-4"><div><Label className="text-xs text-white/52">Asset</Label><Select value={selectedToken?.address} onValueChange={value => { setTokenAddress(value); setReviewing(false); setStage("review"); }}><SelectTrigger className="mt-2 h-10 border-white/[0.1] bg-black/20 text-white"><SelectValue /></SelectTrigger><SelectContent>{tokens.map(token => <SelectItem key={token.address} value={token.address}>{token.symbol} · {token.currency}</SelectItem>)}</SelectContent></Select></div><div><Label className="text-xs text-white/52">Amount</Label><Input value={amount} onChange={event => { setAmount(event.target.value); setReviewing(false); setStage("review"); }} inputMode="decimal" placeholder="0.00" className="mt-2 h-10 border-white/[0.1] bg-black/20 text-white placeholder:text-white/25" /></div>{mode === "withdraw" ? <div><Label className="text-xs text-white/52">Recipient</Label><Input value={recipient} onChange={event => { setRecipient(event.target.value); setReviewing(false); setStage("review"); }} placeholder="0x…" className="mt-2 h-10 border-white/[0.1] bg-black/20 font-mono text-white placeholder:text-white/25" /></div> : null}<div className="rounded-xl border border-[#b3ff3e]/15 bg-[#b3ff3e]/[0.05] p-3 text-xs leading-5 text-white/60"><ShieldCheck className="mr-1 inline h-3.5 w-3.5 text-[#b3ff3e]" />{mode === "deposit" ? "You sign permit, approval, and deposit transactions in your wallet. The server only builds and broadcasts signed transaction bytes." : "You sign the WithdrawIntent and final EIP-1559 transaction in your wallet. Sera’s executor only co-signs the same intent."}</div>{reviewing ? <div className="rounded-xl border border-white/[0.1] bg-black/20 p-3 text-xs leading-5 text-white/60"><p className="font-medium text-white">Confirm vault transaction</p><p className="mt-1">{mode === "deposit" ? `Deposit ${amount || "0"} ${selectedToken?.symbol ?? ""} into Sera Vault.` : `Withdraw ${amount || "0"} ${selectedToken?.symbol ?? ""} to ${recipient || "your connected wallet"}.`}</p></div> : null}<div className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-3 text-xs text-white/60">{stageCopy}</div><Button onClick={() => reviewing ? submit() : setReviewing(true)} disabled={submitting || !tokens.length || stage === "submitted" || stage === "checkingSettlement" || stage === "settled"} className="h-11 w-full rounded-xl bg-[#b3ff3e] text-[#0b1a0e] hover:bg-[#c6ff66]">{submitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Check className="mr-2 h-4 w-4" />}{reviewing ? (mode === "deposit" ? "Confirm & sign deposit" : "Confirm & sign withdrawal") : (mode === "deposit" ? "Review deposit" : "Review withdrawal")}</Button>{stage === "submitted" ? <Button onClick={refreshSettlement} disabled={settlementQuery.isFetching} variant="outline" className="h-10 w-full border-white/[0.1] bg-transparent text-white/75 hover:bg-white/[0.06] hover:text-white">{settlementQuery.isFetching ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}Check Sera settlement</Button> : null}{stage === "submitted" || stage === "settled" ? <Button onClick={() => { setReviewing(false); setStage("review"); setAmount(""); setSettlementId(null); setSettlementDetail(""); }} variant="outline" className="h-10 w-full border-white/[0.1] bg-transparent text-white/75 hover:bg-white/[0.06] hover:text-white">Start another transaction</Button> : null}</div></DialogContent></Dialog>;
}
